              ...
              <Fragment key={index}>
                {
                  category.blogs.map((blog, index2) => (
                    <div key={index2} className="flex flex-col flex-1 max-w-sm gap-3 rounded-lg items-center shadow-lg drop-shadow-lg bg-stone-100">
                      <div className="flex flex-col w-full justify-center items-center">
                        <img src={blog.image.src} alt={blog.image.alt} className="w-full rounded-t-lg h-auto" draggable={false} />
                      </div>
                      <h1 className="text-lg md:text-xl font-bold text-center md:text-left text-neutral-700">{blog.title}</h1>
