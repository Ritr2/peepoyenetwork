    ...
    <main className={`relative flex min-h-screen flex-col items-center mt-16 ${dmSans.className} overflow-x-hidden`}>
      <section className="flex flex-col md:flex-row w-full bg-neutral-200 items-center justify-center px-2 py-12 md:py-24 md:px-20 border-b-4 border-dotted border-neutral-700">
        <div className="flex flex-col justify-center items-center flex-1 gap-2 md:gap-5">
          <h1 className="text-xl md:text-5xl font-bold text-center md:text-left text-neutral-700">START NOW WITH OUR RESOURCES</h1>
          <h2 className="text-sm md:text-lg font-thin text-center md:text-left text-neutral-500">Progress can’t wait. These resources are the perfect way to get started or supplement your growth plan.</h2>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row flex-1 gap-10 md:gap-16">
          {
            data.tools.map((tool, index) => (
              <div key={index} className="flex flex-col flex-1 gap-3 rounded-lg items-center max-w-sm shadow-lg drop-shadow-lg bg-stone-100">
                <div className="flex flex-col w-full justify-center items-center rounded-t-lg">
                  <img src={tool.image.src} alt={tool.image.alt} className="w-full h-auto" draggable={false} />
                </div>
