...
export default function WhoIsThisFor() {
  ...
  return (
    <div className="flex flex-col md:flex-row flex-wrap justify-center gap-5 md:gap-10 w-full">
        {
            data.map((item, index) => (
                <div key={index} className={`flex flex-col items-center gap-5 justify-between w-full md:w-4/12 bg-white rounded-lg ${style.redShadow} p-5`}>
                    ...
                    <div className="flex flex-col w-full items-center gap-5">
                        <h2 className="text-xl md:text-2xl font-bold text-center md:text-left text-neutral-900">{item.h1}</h2>
                        <p className="text-sm md:text-base font-normal text-justify md:text-left text-neutral-700">{item.p}</p>
                    </div>
                </div>
            ))
        }
