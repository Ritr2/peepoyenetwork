...
export default function PaymentForm({ setDataFormVisible, successUrl, loc, stripePriceId, quantity }) {
    ...
    const formHandler = async (e) => {
        ...
        ls.set('phone', phone)
        if (loc === 'ind') {
                makePayment(details, setLoading, setDataFormVisible, successUrl, "dfy", quantity)
        }
        else {
            const products = [
                {
              ...
              StripeapiCall(details, products, 'payment', successUrl, `${url}/dfy-${loc}`, setLoading, "dfy")
        }
    }
            ...
            <AnimatePresence>
                {
                    ...
                    <motion.div
                        ...
                        <motion.div
                            ...
                            <IoClose className='absolute top-1 right-1 md:top-2 md:right-2 text-3xl text-neutral-700 cursor-pointer hover:text-green-600' onClick={() => setDataFormVisible(false)} />
                            <div className='flex flex-col items-center justify-center gap-5'>
                                ...
                                <form className="flex flex-col w-full gap-5" onSubmit={formHandler}>
                                    ...
                                    <button type="submit" className={`flex rounded-lg p-2 text-white bg-green-700 text-base md:text-xl font-bold flex-row items-center justify-center gap-2 w-full  hover:shadow-lg hover:drop-shadow-lg hover:scale-105 active:shadow-none active:drop-shadow-none active:scale-95`}>Make Payment</button>
                                </form>
                            </div>
                        </motion.div>
                    </motion.div>
