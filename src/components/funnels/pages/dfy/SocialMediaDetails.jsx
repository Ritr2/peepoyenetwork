...
export default function SocialMediaDetails() {
  ...
  return (
    <section className='md:px-8 w-full'>
      <div className='flex flex-col gap-3 py-2 md:py-6 z-10 w-full shadow-lg bg-green-600/10 drop-shadow-lg border-2 rounded-3xl border-stone-200'>
                ...
                Object.keys(images).map((key, index) => (
                  <div key={index} className="flex flex-row items-center justify-center gap-5">
                    ...
                    <div className="flex flex-col justify-center">
                      <p className="text-sm md:text-xl tracking-wide font-bold text-left text-neutral-700">{Number(images[key].count)}{images[key].countSuffix}</p>
                      <p className="text-sm md:text-xl tracking-wide font-bold text-left text-neutral-700">{images[key].bottomText}</p>
                    </div>
                  </div>
                ))
              }
