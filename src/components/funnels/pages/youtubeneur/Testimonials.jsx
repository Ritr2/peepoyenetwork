...
export default function Testimonials() {
    ...
    return (
        <div className="flex flex-col md:flex-row flex-wrap justify-center gap-10">
            {
                data.map((item, index) => (
                    <div key={index} className="flex flex-row items-center justify-center w-full md:w-5/12 p-4 bg-white/5 border-2 border-white rounded-lg shadow-lg">
                        <div className="flex flex-col w-8/12 gap-2">
                            <p className=" text-sm md:text-base font-light text-neutral-200">&quot;{item.say}&quot;</p>
                            <div className="flex flex-col justify-center">
                                <h3 className="text-base md:text-lg font-medium text-white">{item.name}</h3>
                                <p className="text-sm md:text-base font-medium text-neutral-200">{item.position}</p>
                            </div>
                        </div>
