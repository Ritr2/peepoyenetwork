...
export default function RibbonBonus({ data }) {

  return (
    <>
      {
        data.map((item, index) => (
          <div key={index} className=" relative flex md:flex-row flex-col gap-5 md:gap-1 border-2 border-neutral-900 rounded-lg p-3 py-6">
            <div className="absolute -top-9 -right-10 md:-top-16 md:-right-16">
              ...
              <span className={`absolute flex h-10 rotate-45 text-xl md:text-3xl font-bold text-white ${style.ribbon}`}>₹ {bonus[item].value}/-</span>
            </div>
            <div className="flex flex-col justify-center gap-2 md:gap-4 w-full md:w-6/12 order-2 md:order-1">
              <h2 className="text-xl md:text-5xl font-bold text-center md:text-left text-neutral-900">{bonus[item].h1}</h2>
            ...
            <div className="flex flex-col items-center justify-between flex-1 order-1 md:order-2">
              <div className="flex flex-col w-11/12">
                <img src={bonus[item].image.src} alt={bonus[item].image.alt} className="w-full" draggable={false} />
              </div>
              <div className="flex flex-col w-full items-center">
                <p className="text-base md:text-2xl font-bold text-center md:text-left text-yellow-600">Total Value: ₹<s>{bonus[item].value}</s>/-</p>
