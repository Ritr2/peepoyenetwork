...
export default function PollAndResult({ data, category, passloading }) {
  ...
  return (
    <div className={`flex flex-col px-5 py-5 pb-16 md:px-24 gap-5 md:py-12 ${sora.className}`}>
      ...
      <form className="flex flex-col items-center gap-5 md:gap-10" onSubmit={handleSubmit}>
        <div className="flex flex-col md:flex-row items-center gap-5 md:gap-5 w-full">
                      ...
                      <>
                        {
                          result ?
                              ...
                              {data.sub_category.name !== 'Expired Polls' &&
                                <div className='flex flex-col w-full items-center justify-center gap-4'>
                                  ...
                                  <div className='flex flex-row w-full items-center justify-center gap-6'>
                                    ...
                                    <a href={`https://api.whatsapp.com/send?text=Share Your opinion Now- ${url}/bekhauf-awaaz/${category}/${data.slug}`} target='_blank' rel="noreferrer" className='h-16 w-16'><img src="https://i.ibb.co/y6rkDdg/Whats-App-icon.webp" alt="whatsapp" className='w-full h-full' /></a>
                            ...
                            <div className="flex flex-row flex-wrap gap-5 justify-center">
                              {
                                data.options.map((option, index) => (
                                  <div key={index} className={`flex flex-col ${option.name.length > 40 ? 'w-full' : 'w-full md:w-[47.5%]'}`}>
                                    <input type="radio" name="optionsquiz" className={`hidden ${style.quizPoll}`} value={option.id} id={`option-${index}`} required />
                                    <label htmlFor={`option-${index}`} className="text-sm md:text-base flex flex-col h-full text-center items-center justify-center gap-2 bg-neutral-200 hover:bg-neutral-300 text-neutral-900 p-2 rounded-lg cursor-pointer">
        ...
        {
          (!result && !productAdView) && <button type="submit" className="bg-neutral-900 hover:bg-neutral-700 text-white px-4 py-2 rounded-lg w-full md:w-4/12">Vote</button>
        }
      </form>
    </div>
  )
